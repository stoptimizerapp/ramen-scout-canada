...
import { absoluteUrl, FULL_CONTENT_INDEXING_ENABLED, pageRobots } from "@/lib/site";
...
export async function generateMetadata({ params }: { params: Promise<{ province: string; city: string; slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    robots: pageRobots(FULL_CONTENT_INDEXING_ENABLED || isRestaurantIndexable(restaurant)),
    ...
    <main className="listing-shell">
      ...
      <div className="listing-layout">
        <article className="listing-content" data-publisher-content>
          ...
          <section id="sources"><p className="section-kicker">Sources &amp; transparency</p><h2>Where these details came from</h2><p>We synthesize facts from official restaurant pages and attributed business data, keep unsupported claims unknown, and keep advertising disabled while additional editorial and schema review continues.</p><div className="source-list">{restaurant.evidence.map((source) => <article id={`source-${source.id}`} key={source.id}><div><strong>{source.id} · {source.publisher || titleCaseToken(source.sourceType || "Source")}</strong><span>{source.sourceType ? titleCaseToken(source.sourceType) : "Source"} · checked {formatDate(source.retrievedAt || source.effectiveDate)}</span>{source.supports?.length ? <small>Supports: {source.supports.map(titleCaseToken).join(", ")}</small> : null}</div>{source.url ? <a href={source.url} rel="nofollow noopener noreferrer" target="_blank">Open source <span aria-hidden="true">↗</span></a> : null}</article>)}</div><p className="source-note">See something outdated? <Link href={`/corrections?restaurant=${encodeURIComponent(restaurant.name)}&id=${restaurant.id}`}>Report a correction</Link>.</p></section>
        </article>
