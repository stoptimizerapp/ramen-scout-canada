...
function StatusFact({ label, value, note }: { label: string; value: string; note?: string }) {
  const unconfirmed = /not confirmed|unknown/i.test(value);
  return <div className="status-fact"><span>{label}</span><strong className={unconfirmed ? "status-unknown" : ""}>{value}</strong>{note ? <small>{note}</small> : null}</div>;
}
    ...
    <main className="listing-shell">
      ...
      <div className="listing-layout">
        <article className="listing-content">
          <section><p className="section-kicker">At a glance</p><h2>What to know about {restaurant.name}</h2><p className="lead-copy">{restaurant.content.editorialDescription}</p><p>{restaurant.content.whyGo}</p></section>
          {(restaurant.content.whatToOrder || restaurant.menu.items.length) ? <section><p className="section-kicker">What to order</p><h2>Menu highlights</h2>{restaurant.content.whatToOrder ? <p>{restaurant.content.whatToOrder}</p> : null}{restaurant.menu.items.length ? <div className="menu-list">{restaurant.menu.items.map((item) => <article key={item.name}><div><h3>{item.name}</h3><p>{[...(item.brothStyle || []), ...(item.tare || []), ...(item.servingStyle || [])].map(titleCaseToken).join(" · ") || "Ramen-family menu item"}</p></div><strong>{item.price !== undefined ? formatMoney(item.price) : "Price not confirmed"}</strong></article>)}</div> : <p className="unknown-callout">Item-level menu details were not confirmed in the latest source check.</p>}</section> : null}
          <section><p className="section-kicker">Broth, noodles &amp; dietary details</p><h2>Confirmed facts and honest unknowns</h2><div className="fact-matrix"><StatusFact label="Tonkotsu" value={triStateLabel(restaurant.taxonomy.tonkotsu, "Confirmed on menu", "Not found on a complete checked menu")} /><StatusFact label="Shoyu" value={triStateLabel(restaurant.taxonomy.shoyu, "Confirmed on menu", "Not found on a complete checked menu")} /><StatusFact label="Miso" value={triStateLabel(restaurant.taxonomy.miso, "Confirmed on menu", "Not found on a complete checked menu")} /><StatusFact label="Tsukemen" value={triStateLabel(restaurant.taxonomy.tsukemen, "Confirmed on menu", "Not found on a complete checked menu")} /><StatusFact label="Vegan ramen" value={veganLabel(restaurant)} note={restaurant.vegan.itemNames.length ? restaurant.vegan.itemNames.join(", ") : undefined} /><StatusFact label="Halal" value={halalLabel(restaurant)} note={restaurant.halal.note} /><StatusFact label="Noodle production" value={noodleLabel(restaurant)} note={restaurant.noodles.scope} /><StatusFact label="Observed ramen prices" value={formatPriceRange(restaurant)} note={restaurant.prices.verifiedAt ? `Checked ${formatDate(restaurant.prices.verifiedAt)}` : undefined} /></div></section>
          ...
          <section><p className="section-kicker">Visit planning</p><h2>Service and accessibility</h2><div className="fact-matrix compact-facts"><StatusFact label="Dine-in" value={triStateLabel(restaurant.services.dineIn, "Available")} /><StatusFact label="Takeout" value={triStateLabel(restaurant.services.takeout, "Available")} /><StatusFact label="Delivery" value={triStateLabel(restaurant.services.delivery, "Available")} /><StatusFact label="Reservations" value={restaurant.reservations.status === "accepted" ? "Accepted" : restaurant.reservations.status === "required" ? "Required" : restaurant.reservations.status === "not_offered_confirmed" ? "Reported unavailable" : "Not confirmed"} /><StatusFact label="Wheelchair-accessible entrance" value={triStateLabel(restaurant.services.wheelchairEntrance, "Reported available")} /><StatusFact label="Wheelchair-accessible seating" value={triStateLabel(restaurant.services.wheelchairSeating, "Reported available")} /></div>{restaurant.content.visitTips ? <p>{restaurant.content.visitTips}</p> : null}{restaurant.content.caveats ? <div className="caveat"><strong>Before you go</strong><p>{restaurant.content.caveats}</p></div> : null}</section>
          {restaurant.content.faqs.length ? <section><p className="section-kicker">Questions answered</p><h2>Helpful details for your visit</h2><div className="faq-list">{restaurant.content.faqs.map((faq) => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p>{faq.verifiedAt ? <small>Checked {formatDate(faq.verifiedAt)}</small> : null}</details>)}</div></section> : null}
          <section id="sources"><p className="section-kicker">Sources &amp; transparency</p><h2>Where these details came from</h2><p>We synthesize facts from official restaurant pages and attributed business data, keep unsupported claims unknown, and require editorial review before a page becomes indexable or ad-enabled.</p><div className="source-list">{restaurant.evidence.map((source) => <article key={source.id}><div><strong>{source.publisher || titleCaseToken(source.sourceType || "Source")}</strong><span>{source.sourceType ? titleCaseToken(source.sourceType) : "Source"} · checked {formatDate(source.retrievedAt || source.effectiveDate)}</span></div>{source.url ? <a href={source.url} rel="nofollow noopener noreferrer" target="_blank">Open source <span aria-hidden="true">↗</span></a> : null}</article>)}</div><p className="source-note">See something outdated? <Link href={`/corrections?restaurant=${encodeURIComponent(restaurant.name)}&id=${restaurant.id}`}>Report a correction</Link>.</p></section>
        </article>
