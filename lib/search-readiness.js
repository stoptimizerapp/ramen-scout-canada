...
export function isRestaurantSearchReady(restaurant, now = Date.now()) {
  ...
  const validSupplement = supplement.crawl4aiSuccess === true
    && supplement.extractionMethod === "crawl4ai_normalized_markdown"
    ...
    && (supplement.corroboration?.ramenTerms || 0) >= 2
    && ((supplement.corroboration?.matchedItems || 0) >= 1 || (supplement.corroboration?.priceSignals || 0) >= 2)
  ...
  return qualityScore >= SEARCH_READINESS.minimumQualityScore
    ...
    && REQUIRED_CONTENT_GATES.every((gate) => gates[gate] === "yes" || gate === "decisionData" && validSupplement)
