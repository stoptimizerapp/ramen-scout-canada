...
export function validateSearchReadinessEnrichments(payload) {
  ...
  for (const record of payload.records) {
    ...
    for (const item of record.items) {
      ...
      if (!Array.isArray(item.evidenceRefs) || !item.evidenceRefs.length) throw new Error(`Menu enrichment item ${item.name} requires evidence references.`);
    }
    ...
    if (record.prices) {
      ...
      if (!Array.isArray(record.prices.evidenceRefs) || !record.prices.evidenceRefs.length) throw new Error(`Menu enrichment prices for ${record.restaurantId} require evidence references.`);
    }
...
export function applySearchReadinessEnrichment(restaurant, enrichment) {
  if (!enrichment) return restaurant;
  if (restaurant.menu?.url !== enrichment.replacesMenuUrl) throw new Error(`Menu enrichment for ${restaurant.name} is stale: the previous menu URL changed.`);
  ...
  if (unresolved.length) throw new Error(`Menu enrichment for ${restaurant.name} has unresolved evidence refs: ${unresolved.join(", ")}.`);
  return {
    ...restaurant,
