...
export function validateSearchReadinessEnrichments(payload) {
  ...
  for (const record of payload.records) {
    ...
    if (typeof record.replacesMenuUrl !== "string") throw new Error(`Previous menu URL for ${record.restaurantId} must be a string.`);
    if (record.replacesMenuUrl) ensureHttps(record.replacesMenuUrl, `Previous menu URL for ${record.restaurantId}`);
    ...
    if (record.reservations) {
      if (!["accepted", "required", "not_offered_confirmed", "unknown"].includes(record.reservations.status)) throw new Error("Invalid reservation status");
      validateEvidenceRefs(record.reservations.evidenceRefs, `Reservations for ${record.restaurantId}`);
    }
...
export function applySearchReadinessEnrichment(restaurant, enrichment, { allowPendingSupplementEvidence = false } = {}) {
  ...
  return {
    ...
    reservations: enrichment.reservations ? { ...restaurant.reservations, ...enrichment.reservations, confidence: "high" } : restaurant.reservations,
